...
class SignTool_Message(inkex.Effect):
  def __init__(self):
  ...
  def draw_message(self, message, fontsize, fontheight, xleft_in, ytop_in, parent):
    ...
    while i < len(msg_list):
      ...
      le = msg_list[i]
      if le[0] == 'letter':
        (le_left, le_width, le_right) = self.letter_size(le[1], fontsize, fontheight)
        ...
        xleft = xleft + self.fraction_width( le[1], fontsize, fontheight)
        
      elif le[0] == 'distance':
        if msg_list[i-1][0] == 'letter' and msg_list[i+1][0] == 'letter':
          xleft = xleft + self.distance_width(le[1], fontsize, fontheight, msg_list[i-1][1], msg_list[i+1][1])
        else:
          xleft = xleft + self.distance_width( le[1], fontsize, fontheight, '/', '/')
      
      elif le[0] == 'cent':
        (le_left, le_width, le_right) = self.letter_size('cent', fontsize, fontheight)
  ...
  def draw_fraction(self, message,fontsize, fontheight, xleft, ytop, parent):
    ...
    self.draw_letter(message[0], fontsize, fontheight, xleft, ytop_n, parent)
    ...
    xleft_slash = xleft + self.letter_size(message[0],fontsize, fontheight)[1] + \
                 space_width * 9 / 16 - self.letter_size(message[1],fontsize, fontheight)[1] /2
    
    if message[2] == '4':
      xleft_slash = xleft_slash + space_width * 2 / 16
    
    ytop_slash = ytop_n + fontheight / 2.0
    self.draw_letter(message[1], fontsize, fontheight, xleft_slash, ytop_slash, parent)
    
    # draw denominator
    xleft_d = xleft + whole_width - \
                self.letter_size(message[2], fontsize, fontheight)[1]
    ytop_d = ytop + fontheight / 4.0
    self.draw_letter(message[2], fontsize, fontheight, xleft_d, ytop_d, parent)
  ...
  def draw_letter(self, letter, fontsize, fontheight, xleft, ytop, parent):
    ...
    if self.options.bDrawBox:
      style_rect = {
          ...
          'fill': 'none'}
      self.draw_SVG_rect(xleft, ytop , w_letter, fontheight, style_rect, letter , parent)
