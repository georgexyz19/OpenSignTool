...
#  
#  Copyright June 2018 George Zhang <george@X230>
#  
...
class SignTool_Arrow(inkex.Effect):
  def __init__(self):
  ...
  def effect(self):
    ...
    layer = self.create_layer(self.document.getroot(), 'arrows')
    style = {
      'stroke': '#000000',
    ...
    ratio = self.unittouu('1in')
    dy = 0 
    for r in tb:
      r0, r1, r2, r3, r4 = (float(r[0]), float(r[1]), float(r[2]), float(r[3]), float(r[4])) 
      dy += ratio * r0 /2 + 2 * ratio *r1
      self.draw_SVG_arrow(0, dy, r0, r1, r2, r3, r4, style, 'arrow', layer)
  ...
  def draw_SVG_arrow(self, x, y, A, B, C, D, E, style, name, parent):
    ...
    pathd += 'A %s %s %s %s %s %s %s ' % (E, E, 0, 0, 0, A + E * math.cos(b), B + E * math.sin(b))

    pathd += 'L %s %s ' % (A + C, 0)
    
    pathd += 'L %s %s ' % (A + E * math.cos(b), -1 * (B + E * math.sin(b)) )
    pathd += 'A %s %s %s %s %s %s %s ' % (E, E, 0, 0, 0, A + E * math.cos(d), -1 * ( B + E * math.sin(d)) )
    pathd += 'L %s %s ' % (x0, -1 * y0)
    pathd += 'L %s %s ' % (0, -1 * A / 2)
    
    pathd += 'Z'
    
    pp = simplepath.parsePath(pathd)
    simplepath.translatePath(pp, x, y)
    pathd = simplepath.formatPath(pp)
